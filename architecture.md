# Project A: AI-Powered Asset Search - Architecture Documentation
...
async def auto_tag_asset(asset_id: str):
    ...
    tags = await gpt4v_client.analyze_image(
        image_url=asset.url,
        prompt="Generate 5-10 descriptive tags. Return JSON array."
    )
...
async def test_similarity_search():
    ...
    assert all(
        r.similarity_score >= 0 and r.similarity_score <= 1 
        for r in results
    )
```
